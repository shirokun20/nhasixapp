class TitleParserUtils {
  // Hoisted: getBaseTitle runs 2x per row on full-library grouping passes.
  static final RegExp _chapterMarkerRegExp = RegExp(
    r'[\s\-]*(?:chapter\b|ch\.|part\b|vol\.|volume\b|episode\b|ep\.|hen\b|編|#).*$',
    caseSensitive: false,
  );
  static final RegExp _trailingNumberRegExp = RegExp(r'\s+\d+(?:\.\d+)?$');
  static final RegExp _trailingDashColonRegExp = RegExp(r'[\s\-:]+$');
  ...
  static String getBaseTitle(String fullTitle) {
    ...
    String baseTitle = fullTitle.replaceAll(_chapterMarkerRegExp, '').trim();
    ...
    final withoutTrailingNum =
        baseTitle.replaceAll(_trailingNumberRegExp, '').trim();
    ...
    baseTitle = baseTitle.replaceAll(_trailingDashColonRegExp, '').trim();
