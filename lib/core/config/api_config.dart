...
class ApiConfig {
  ...
  static String get nhentaiApiBase =>
      _nhentaiConfig?.api?.apiBase ?? 'https://nhentai.net/api';

  /// nhentai website base URL (for fallback scraping)
  static String get nhentaiWebBase =>
      _nhentaiConfig?.baseUrl ?? 'https://nhentai.net';

  /// API request timeout in milliseconds
  static int get apiTimeout => _nhentaiConfig?.api?.timeout ?? 30000;

  /// Enable automatic fallback to HTML scraper if API fails
  static bool get enableApiFallback =>
      _nhentaiConfig?.features?.related ?? true;

  /// Maximum retry attempts for API requests
  static int get maxRetryAttempts =>
      _nhentaiConfig?.network?.retry?.maxAttempts ?? 3;

  /// Delay between retries in milliseconds
  static int get retryDelayMs =>
      _nhentaiConfig?.network?.retry?.delayMs ?? 1000;

  // ============ Rate Limiting ============

  /// Minimum delay between API requests in milliseconds
  static int get minRequestDelayMs =>
      _nhentaiConfig?.network?.rateLimit?.minDelayMs ?? 200;

  /// Maximum requests per minute
  static int get maxRequestsPerMinute =>
      _nhentaiConfig?.network?.rateLimit?.requestsPerMinute ?? 60;
