...
void main() {
  group('DownloadStorageUtils.getSafeTitleFromMetadata', () {
    test('returns correct title when metadata is completely valid', () {
      ...
      final result = DownloadStorageUtils.getSafeTitleFromMetadata(
          metadata, 'folderHash123');
    ...
    test(
        'formats the original id when title is suspected ciphertext (no spaces and > 50 length)',
        () {
    ...
    test(
        'returns unaffected title if it is just a short unspaced word (not ciphertext)',
        () {
      final metadata = {
        'title': 'ShortNoSpaces',
    ...
    test(
        'handles empty original id gracefully and falls back to formatting contentId (folder name)',
        () {
      final metadata = {
        'title': '',
        'id': '',
