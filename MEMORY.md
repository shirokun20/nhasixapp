...
|---|---|---|---|---|
| 2026-06-09 | Codex | offline-library controls padding + scroll behavior | ✅ Done | Kept offline library controls visible while scrolling, removed the hide/show-on-scroll behavior, and increased bottomsheet spacing so `Sort`/`Filter` and the info summary no longer feel cramped. Verified with targeted `fvm flutter analyze` and `fvm flutter test test/widget/presentation/widgets/offline_content_body_test.dart`. |
| 2026-06-09 | Codex | offline-library bottomsheet active state polish | ✅ Done | Matched the Sort/Filter active states to the sidebar treatment, animated the header title between sections, and strengthened selected option tiles in the offline options sheet. Verified with targeted `fvm dart format`, `fvm flutter analyze`, and `fvm flutter test test/widget/presentation/widgets/offline_content_body_test.dart`. |
