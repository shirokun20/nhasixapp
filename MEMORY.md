...
|---|---|---|---|---|
| 2026-06-25 | Codex | Home cover combined read/download border | ✅ Done | Updated `MainFeaturedCard` and `MainGridCard` to query local history and render a stacked border when a card is both read and downloaded; `ContentCard` now also supports a combined read/download border when `readProgress` is present. Verified with `dart format` and `flutter analyze`. |
