...
|---|---|---|---|---|
| 2026-07-01 | Claude Code | Search Form UI Parity | Done | Advanced Filters toggle panel in `DynamicFormSearchUI`. Config `options` upgrade `{name, value}` objects. Fixed `_collectEncodedQueryParts` missing `sort` type. Main screen no longer overrides saved filter sortBy. Sort sync between main+search screens. Sort chip selector in `QueryStringSearchUI`. Migrated crotpedia, komikcast, spyfakku, tooncubus, shirodoujin, vihentai from `searchConfig` → `searchForm`. Added `SearchFormOptionConfig` dual String/Map fromJson. AuthConfig null safety. 464/464 tests. |
