...
|---|---|---|---|---|
| 2026-06-29 | Codex | HentaiNexus cover stabilization + ToonCubus reader/tag runtime fix | ✅ Done | HentaiNexus detail cover scraping now uses `meta[property="og:image"]` so cover URLs come straight from page metadata instead of being guessed from reader image extensions. Also locked in the recent HentaiNexus reader fixes: page-URL dedupe with format priority, detail-cache bypass, continuous-scroll heavy-image guard, continuous-scroll height fallback, and safer detail-header cover fallback. For ToonCubus, updated the local config/runtime to match the real Blogger fixtures: home uses `max-results=16`, search uses `max-results=12`, label/tag pages use `/search/label/{tag}`, detail exposes a real `Baca Online` chapter list, and reader fetching now follows the explicit `tooncubus-read.my.id` link before scraping final `imgbox` images. Added focused `kuron_generic` fixture tests for ToonCubus home pagination, detail chapter extraction, reader-link handoff, and label-route tag search. |
...
| 2026-06-27 | Codex | Offline metadata `content_id` resync fix | ✅ Done | Fixed offline DB rebuild for hashed download folders (`komiktap`, `crotpedia`, and similar native-worker downloads). `DownloadStorageUtils` / `OfflineContentManager` now read native `metadata.json` keys like `content_id` and `source`, so full offline resync rebuilds DB rows with the real chapter ID instead of the elegant/hash folder name (e.g. `10vlmmznl1`, `11k38msg3d`). Added regression tests for both metadata sync and card download-status matching. |
| 2026-06-27 | Codex | Tag page + offline card revamp | ✅ Done | `content_by_tag` now reuses the home-style featured/grid cards instead of the older generic card renderer. Offline library group cards were also remade with clearer offline/read status pills, stronger cover-first hierarchy, and extra bottom grid padding so the last row does not sit too close to the floating sort button. |
...
| 2026-06-24 | Codex | ZIP import multiple + local path flattening | ✅ Done | Added native multi-ZIP picker support and wired Dart fallback to single-pick when multi-select is unavailable. ZIP extraction now flattens image paths to the filename leaf so nested archive folders no longer create `[folder]/[image]` under `images/`. Backup sync now honors the active source filter: `local` gets a source-folder-only resync with stale local DB rows cleared first, while `all` keeps scanning all source roots. Verified with targeted `fvm flutter test test/unit/core/utils/offline_content_manager_metadata_sync_test.dart test/unit/domain/usecases/import_zip_usecase_test.dart` and `fvm flutter analyze` on the touched Dart files. |
...
| 2026-06-21 | Codex | Offline duplicate chapter detail fix | ✅ Done | Fixed duplicate offline detail rows when the DB contains multiple completed downloads with the same source/title/local path but different IDs. `ContentGroup` now dedupes items by source + normalized title + cover directory, `OfflineSearchCubit` computes grouped size/progress from the unique item list, and `OfflineSeriesDetailScreen` applies the same dedupe in its direct storage fallback. Verified focused offline search cubit test and targeted analyze. |
| 2026-06-21 | Codex | archive tabbed-multilang + search-runtime-autowiring | ✅ Done | Added changelog notes for the chapter language lane and search runtime wiring work, then archived `openspec/changes/tabbed-multilang-chapters` and `openspec/changes/search-runtime-autowiring` to `openspec/changes/archive/2026-06-21-*`. |
| 2026-06-21 | Codex | Detail chapter preview UI polish | ✅ Done | Polished the chapter preview section on the detail page (`DetailChapterSection`) to match the improved bottom sheet direction: lighter container, quieter header, compact language chips with stronger selected state, slimmer preview rows, scanlation/date/read-progress subtitle, and less heavy read/download actions. Verified focused analyze and detail chapter widget test. |
| 2026-06-21 | Codex | Chapter bottom-sheet UI polish | ✅ Done | Modernized `ChapterListBottomSheet` after MangaDex paging follow-up: language chips now sit in a fixed rail with stronger selected styling, header shows selected language and loaded count, rows show scanlation group/date subtitle when available, read/completed border states are clearer, and MangaDex load-more uses a stronger footer action (`Load 100 more ...`). Verified focused analyze and detail chapter widget test. |
| 2026-06-21 | Codex | MangaDex chapter lane bottom-sheet paging | ✅ Done | Follow-up to `tabbed-multilang-chapters`: `View all chapters` now opens the bottom sheet scoped to the selected chapter language lane. MangaDex detail chapter config sets `autoPaginate=false`, and the generic REST adapter honors it so detail load stops at the first chapter page instead of fetching all offsets. `ChapterListBottomSheet` adds MangaDex-only `Load more` that fetches the next page for the active `translatedLanguage[]` lane and de-dupes chapters. Verified JSON, targeted analyze, detail widget test, and `packages/kuron_generic` MangaDex integration test. |
| 2026-06-21 | Codex | tabbed-multilang-chapters implementation | ✅ Done | Added shared chapter language presentation (`ChapterLanguagePresenter`) for alias normalization, unknown fallback, lane ordering, and selected-lane preservation. Detail chapter preview and `ChapterListBottomSheet` now show language chips only for multi-language chapter sets while keeping single-language lists non-tabbed. Reader route extras carry active chapter language into reader defaults, and reader same-language navigation uses the shared normalizer. Verified with targeted analyze plus unit/widget tests for presenter, reader route payload, and detail chapter UI. |
...
| 2026-05-31 | Copilot | reader-ux-revamp — all 35 tasks (§1–§10) | ✅ Done | Completed full reader UX revamp: §1 `TapDirection` enum + `tapDirection` field in `ReaderSettingsEntity` model + repo layer. §2 `tapDirection` in `ReaderState` + `setTapDirection()` in `ReaderCubit`. §3 `_isNextTap/_isPrevTap` helpers replacing hardcoded tap zones in singlePage+verticalPage modes. §4 Settings panel redesign with SegmentedButton for mode, tap direction (hidden in continuousScroll), zoom + screen-on toggles wired to cubit. §5 `_evictDistantPages()` — evicts NetworkImage providers outside ±4 window via `addPostFrameCallback`; skips offline. §6 Bidirectional prefetch (1 page back + 3 forward) + dual-timestamp throttle (skip if 2 changes < 300ms). §7 Bottom bar redesign: `Slider` + prev/next buttons + mode icon, all in a `BackdropFilter` blur container. §8 Top bar redesign: blur background, compact icons, `TextOverflow.ellipsis` title. §9.1 Page transition curve changed to `Curves.easeOutCubic` + 200ms. §9.2–9.3 New `ChapterOpenOverlay` widget (fade-in, auto-dismiss 2s, backdrop blur) wired into `_buildReader` Stack. All 301 existing tests pass. `fvm flutter analyze lib/` → No issues. |
