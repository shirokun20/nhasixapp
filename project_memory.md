...
|---|---|---|---|---|
| 2026-03-29 | Codex | Nhentai API v2 endpoint migration | ✅ Done | Updated the active config-driven nhentai runtime only: bundled `nhentai-config.json` now points to list=`/api/v2/galleries?page={page}`, search=`/api/v2/search?...`, detail/comments=`/api/v2/galleries/{id}?include=comments`, related=`/api/v2/galleries/{id}/related`. Confirmed live JSON via `curl`: list/search now return compact items (`english_title`, `japanese_title`, relative `thumbnail`, `tag_ids`), while detail returns `title`, `cover`, `thumbnail`, `pages`, and embedded `comments`. Added nhentai-specific compatibility in `GenericRestAdapter` to resolve v2 titles, relative CDN paths, and page image URLs, and adjusted comments parsing to read `$.comments[*]`. Kept legacy `kuron_nhentai` / `NhentaiApiClient` helpers untouched because nhentai runtime is now fully config-driven. Verified `fvm dart analyze` clean. Targeted Flutter schema test could not validate config because the existing test fixture path resolution fails before assertions. |
