...
| 2026-04-06 | Codex | MangaDex author/artist tag fix + EHentai language fallback fix | ✅ Done | Live check confirmed MangaDex `authorOrArtist` works; fix preserves UUID tag navigation in detail screen instead of discarding non-numeric IDs, and EHentai no longer defaults missing language to `english`; regression tests added for both adapters. |
