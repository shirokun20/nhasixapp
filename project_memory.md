...
|---|---|---|---|---|
| 2026-03-29 | Codex | Hitomi favorite cover header fix | ✅ Done | Fixed Favorites cover rendering for Hitomi: `FavoritesScreen` now resolves per-source image headers from `ContentSourceRegistry` and passes them into `ContentCard.buildImage()`, and the shared helper now uses `ProgressiveImageWidget` instead of raw `CachedNetworkImage` so custom `httpHeaders` are honored consistently. This restores Hitomi favorite cover loading where CDN requests require `Referer`/`User-Agent`. Verified with `fvm dart analyze` on touched files. |
