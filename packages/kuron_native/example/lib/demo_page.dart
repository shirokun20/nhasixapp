...
enum PipelineState { welcome, imageLoaded, detecting, detected, buildingMosaic, translating, translated, error }
...
class _DemoPageState extends State<DemoPage> {
  ...
  String _providerBaseUrl = 'http://192.168.0.6:20128/v1/chat/completions';
  String _providerModel = 'ocg/minimax-m3';
  String _providerKey = '';
...
Future<Uint8List?> _buildMosaicIsolate(
    Uint8List pageBytes, List<Map<String, dynamic>> boxMaps, int imgW, int imgH) async {
  return Isolate.run(() {
    ...
    for (final m in boxMaps) {
      ...
      img.drawString(lbl, '${boxMaps.indexOf(m) + 1}', font: font, x: 4,
          y: (crop.height ~/ 2) - 12, color: img.ColorRgba8(255, 0, 0, 255));
      final combined = img.Image(width: lblW + crop.width, height: crop.height);
    ...
    final mh = chips.fold(0, (sum, c) => sum + c.height) + gap * (chips.length - 1);
    final mosaic = img.Image(width: mw, height: mh);
    var yo = 0;
    for (final c in chips) { img.compositeImage(mosaic, c, dstX: 0, dstY: yo); yo += c.height + gap; }
  ...
  Future<void> _loadTestImage() async {
    ...
    try {
      ...
      setState(() { _state = PipelineState.error; _error = 'Failed to load test image.'; });
    }
  }
  ...
  Future<void> _runTranslation() async {
    ...
    _logm('boxes=${boxes.length}, pageBytes=${_pageBytes != null}, manual=$_isManualMode');
    if (boxes.isEmpty || _pageBytes == null) { _logm('SKIP: no boxes or no image'); return; }
    ...
    try {
      ...
      final jpeg = await _buildMosaicIsolate(_pageBytes!, boxMaps, _imgW, _imgH);
      if (jpeg == null) { _logm('FAILED: mosaic null'); setState(() { _state = PipelineState.error; _error = 'Translate gagal'; }); return; }
      ...
      try {
        ...
        if (resp.statusCode != 200) {
          ...
          setState(() { _state = PipelineState.error; _error = 'Translate gagal'; });
          return;
        }
      ...
      _logm('FAILED: $e');
      setState(() { _state = PipelineState.error; _error = '$e'; });
    }
  }

  String _stylePrompt(String style) {
    switch (style) {
      case 'literal': return 'Translate accurately and literally. Preserve exact tone.';
      case 'natural': return 'Use natural, fluent Indonesian as a native speaker.';
      case 'gaul': return 'Use informal Indonesian slang: gue/lo instead of saya/kamu.';
      case 'perwibuan': return 'Gunakan bahasa Indonesia keraton: hamba, paduka, berkenan.';
      case 'kasar': return 'Gunakan kata kasar ringan: anjir, bangsat, goblok.';
      case 'emakGosip': return 'Gaya emak gosip: Eh tau nggak sih!, Ya ampun!';
      case 'bapack': return 'Gaya bapak-bapak: ngab, suhu, kopi dulu, mantap jaya.';
      case 'betawi': return 'Gaya Betawi: aye, elu, kagak. Plus pantun pendek.';
      default: return '';
    }
  }
  ...
  void _toggleManualMode() {
    setState(() { _isManualMode = !_isManualMode; _isDrawing = false; });
    _logm('Manual bubble mode ${_isManualMode ? "ON" : "OFF"}');
  }

  void _onDrawStart(Offset p) { if (!_isManualMode) return; _drawStart = p; _drawEnd = p; _isDrawing = true; }
  void _onDrawMove(Offset p) { if (!_isManualMode || !_isDrawing) return; _drawEnd = p; setState(() {}); }
  void _onDrawEnd(Offset p) {
    ...
    if (r.width < 20 || r.height < 20) {
      _logm('Area too small, skipped.'); _drawStart = _drawEnd = null; return;
    }
    ...
    _manualBoxes.add(BubbleBox(
      x: (r.left * sx).round(), y: (r.top * sy).round(),
      w: (r.width * sx).round(), h: (r.height * sy).round(),
      confidence: 1.0,
    ));
  ...
  void _removeDetected(int i) { if (i >= 0 && i < _detectedBoxes.length) { _detectedBoxes.removeAt(i); setState(() {}); } }
  void _removeManual(int i) { if (i >= 0 && i < _manualBoxes.length) { _manualBoxes.removeAt(i); setState(() {}); } }
  void _clearAll() { _resetPipeline(); _logm('All reset'); setState(() => _state = PipelineState.imageLoaded); }
  ...
  String _statusText() {
    switch (_state) {
      case PipelineState.welcome: return 'Tap "Load Test Image"';
      case PipelineState.imageLoaded: return 'Image loaded. Run detection.';
      case PipelineState.detecting: return 'Detecting bubbles...';
      ...
      case PipelineState.buildingMosaic: return 'Building mosaic...';
      case PipelineState.translating: return 'Translating...';
      case PipelineState.translated:
        return 'Done! ${_translation?.bubbles.where((b) => !b.isSkipped).length} translated';
      case PipelineState.error: return 'Error: $_error';
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: _state == PipelineState.welcome ? null : [
          IconButton(icon: const Icon(Icons.refresh), tooltip: 'Reset', onPressed: _clearAll),
        ],
      ),
  ...
  Widget _buildWelcome(ThemeData t) => Center(
    child: Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text('AI Translation Pipeline', style: t.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
        ...
        FilledButton.icon(onPressed: _loadTestImage, icon: const Icon(Icons.image), label: const Text('Load Test Image')),
      ]),
    ),
  );
  ...
  Widget _buildMain(ThemeData t) {
      ...
      child: Row(children: [
        Expanded(child: Text(_statusText(),
            style: t.textTheme.bodySmall?.copyWith(
              color: _state == PipelineState.error ? t.colorScheme.error : t.colorScheme.onSurfaceVariant,
            ))),
        ...
        Padding(
          ...
          child: TextButton(
            ...
            child: Text(_providerModel.split('/').last,
    ...
    Widget imgArea = Expanded(
      child: LayoutBuilder(builder: (context, constraints) {
        ...
        if (_pageImage == null || availW <= 0 || availH <= 0) return const SizedBox();
          ...
          child: Stack(
            ...
            children: [
              ...
              if (_isDrawing && _drawStart != null && _drawEnd != null)
                Positioned.fill(child: IgnorePointer(child: CustomPaint(painter: _RectPainter(
                  rect: Rect.fromPoints(_drawStart!, _drawEnd!), color: Colors.green,
                )))),
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Row(children: [
          ...
          SizedBox(
            ...
            child: DropdownButtonFormField<String>(
              ...
              items: ['Indonesia','English','Japanese','Korean','Chinese','Arabic','Thai','Russian','Spanish','Portuguese','French']
          ...
          Expanded(
            child: TranslationStylePicker(selectedStyle: _style, onChanged: (s) {
            setState(() => _style = s);
            _logm('Style: $s');
            if (_state == PipelineState.translated) _runTranslation();
          }, compact: true),
        ...
        Row(children: [
          if (_state == PipelineState.detected)
            TextButton.icon(onPressed: _toggleManualMode,
              icon: Icon(Icons.edit, size: 18, color: _isManualMode ? Colors.green : null),
              label: Text(_isManualMode ? 'Manual ON' : 'Manual',
                style: TextStyle(color: _isManualMode ? Colors.green : null, fontWeight: _isManualMode ? FontWeight.bold : null)),
            ),
          const Spacer(),
          TextButton.icon(
            onPressed: () => _showProviderTestSheet(context),
          ...
          if (_state == PipelineState.translated)
            TextButton.icon(onPressed: () => setState(() => _overlayVisible = !_overlayVisible),
              icon: Icon(_overlayVisible ? Icons.visibility : Icons.visibility_off, size: 18),
    ...
    final logPanel = Container(
      ...
      child: ListView(
        ...
        children: _log.reversed.take(20).map((m) => Text(m,
          style: TextStyle(fontSize: 10, fontFamily: 'monospace',
            color: m.startsWith('FAIL') ? t.colorScheme.error : t.colorScheme.onSurfaceVariant),
        )).toList(),
      ),
    );
...
class _RectPainter extends CustomPainter {
  ...
  void paint(Canvas c, Size s) {
    final p = Paint()..color = color.withValues(alpha: 0.3)..style = PaintingStyle.fill;
    c.drawRect(rect, p);
    p..color = color..style = PaintingStyle.stroke..strokeWidth = 2;
    c.drawRect(rect, p);
  }
  @override
...
class _ProviderTestSheetState extends State<_ProviderTestSheet> {
    ...
    return Padding(
      ...
      child: SingleChildScrollView(
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Test AI Provider', style: t.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
          ...
          DropdownButtonFormField<AiProviderType>(
            ...
            items: AiProviderType.values.map((t) => DropdownMenuItem(value: t, child: Text(t.name))).toList(),
            onChanged: (v) {
              setState(() {
                _selectedType = v!;
  ...
  Future<void> _testProvider() async {
    ...
    try {
      if (_selectedType == AiProviderType.gemini) {
        ...
        _result = await _testOpenAICompatible(baseUrl, apiKey, model, _selectedType);
      }
    } catch (e) {
      _result = '✗ Error: $e';
    }
  ...
  Future<String> _testOpenAICompatible(
      String baseUrl, String apiKey, String model, AiProviderType type) async {
    ...
    if (type == AiProviderType.gemini) {
      body = {
        'contents': [{
          'parts': [{'text': 'Translate the following Japanese text to Indonesian. Return ONLY the translation, no explanation:\n$sampleText'}]
        }]
      };
    } else {
      body = {
        ...
        'messages': [
          {
            ...
            'content': [
              {
                'type': 'text',
                'text': 'Translate the following Japanese text to Indonesian. Return ONLY the translation, no explanation:\n$sampleText'
              }
            ]
          }
        ],
